(function () {
  ...
  /* Куда уходит заявка.
     'form/submit.php' — приёмник на своём хостинге: он пишет заявку в базу
     на сервере в России и только потом шлёт уведомления. Включается,
     когда в репозитории заведены секреты базы, бота и почты.
     Пока их нет, заявки принимает прежний адрес на Cloudflare — иначе
     уведомления перестали бы приходить. Переключение — эта одна строка. */
  ...
  // Свой приёмник лежит в корне сайта: на страницах направлений к его
  // адресу нужен «../». Полный адрес со схемой дополнять не нужно.
  if (!/^https?:/i.test(LEAD_ENDPOINT)) {
